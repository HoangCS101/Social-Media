    ...
    if ($vote) {
        if ($vote->is_vote === 0) {
            if ($type === 'upvote') {
                ...
                $forumVote->save();
                echo json_encode(['success' => true, 'newvote' => $forumVote->total_vote]);
                exit;
                return;
            } else {
                $vote->is_vote = 1;
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($forumVote->save()) {
                ...
                echo json_encode(['success' => false, 'newvote' => $forumVote->total_vote]);
                exit;
            }
        }
    }
...
<div class="flex flex-wrap gap-4">
    <div>
        ...
        <div class="bg-white p-6 flex flex-col items-center space-y-4">
            <!-- Upvote Button -->
            <button id="upvote"
                class="w-10 h-10 flex items-center justify-center rounded-full border-2 border-gray-400 text-gray-600 hover:bg-green-500 hover:text-white transition duration-300 text-2xl">
                ▲
            </button>
            ...
            <!-- Downvote Button -->
            <button id="downvote"
                class="w-10 h-10 flex items-center justify-center rounded-full border-2 border-gray-400 text-gray-600 hover:bg-red-500 hover:text-white transition duration-300 text-2xl">
                ▼
            </button>
        </div>
